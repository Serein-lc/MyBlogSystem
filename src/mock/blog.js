...
Mock.mock("/api/blogtype", "get", {
    ...
    msg: "",
    "data|10-20": [{
        "id|+1": 1, //唯一id，自增，从1开始自增
        name: "分类@id", //分类名字 将id加入到分类后面
        "articleCount|0-300": 0, //该分类下文章的数量,随机生成0-300条数据，后面只要给个数字就行
        "order|+1": 1 //分类数据那个在前面那个在后面
    }]
...
// 模拟博客分页数据   因为这里的请求地址可能带有参数，所以不能直接写死一个地址，要使用正则表达式
// /^\/api\/blog(\?.+)?$/的意思是以/api/blog开头 ?必须出现一次，后面就是任意字符
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function(options) {
    ...
    const data = {
        ...
        [`rows|${limit}`]: [{
            id: "@guid", //全球唯一id
            title: "@ctitle(10,40)", //生成一个中文标题
            description: "@cparagraph(1,10)", //生成一个中文段落，有一到十句话
            category: {
                // 所属分类
                "id|1-10": 0,
                name: "分类@id"
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", // 获取单个博客
    {
        ...
        data: {
            ...
            "thumb|1": [Mock.Random.image('300x250', "#008c8c", "#fff", "text"), null],
...
Mock.mock(/^\/api\/comment(\?.+)?$/, function(options) {
    //querystring模块提供用于解析和格式化URL查询字符串的实用程序。
